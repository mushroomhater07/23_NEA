    ...
    public async Task<string> GetData(bool postdata, string sql, string argument ) {
        try
        {
            ...
            if(postdata)
            {
                ...
                response = await client.PostAsync(new Uri("https://unity.shalevportal.ml"), new FormUrlEncodedContent(values));
            }
            else response = await client.GetAsync("https://unity.shalevportal.ml/date.php");
            response.EnsureSuccessStatusCode();
            var responseBody = await response.Content.ReadAsStringAsync();
            return responseBody;
        }catch (HttpRequestException e){ return "";}
    }
}
