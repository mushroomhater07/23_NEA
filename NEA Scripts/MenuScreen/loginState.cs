...
public class loginState : MenuPanels
{
    ...
    {
        Application.OpenURL("http://localhost:3000/register");
    }
    public void Login()
    {
        ...
        {
            infoHandler(true, "email field is blank");
            return;
        }
        ...
        Debug.Log(Math.Floor(hexadeciamToDenary(result[0])/100000000f)== Math.Floor(hashFunction(PasswordField.text, int.Parse(result[1]))/100000000f));
        Debug.Log(Math.Floor(hexadeciamToDenary(result[0])/100000000f));
        Debug.Log( Math.Floor(hashFunction(PasswordField.text, int.Parse(result[1]))/100000000f));
        try
        {
        ...
        {
            infoHandler(true, "no user of this email found");
        }
